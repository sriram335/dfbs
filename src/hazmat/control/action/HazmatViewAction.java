...
public class HazmatViewAction extends ControlAction
{
    ...
    protected static void setFilterSession(HttpServletRequest request,HazmatPermitViewForm filterForm) throws Exception
    {
        ...
        else if(filter.equals("byPermitNumber")){
            System.out.println("Printing Shipment Number" + filterForm.getShipPermitNumber());
            filterSession.setValue(filterForm.getShipPermitNumber());
        }
    ...
    {
        HttpSession session = request.getSession();
        Log.log("ACTION_LAYER", DHSLogLevel.INFO, "HAZMAT VIEW ACTION",
                "SEARCHSHIPMENTS", "Shipment ID: " + "IN LINE 190 Got session" + session.toString());
        HsUser user = (HsUser) session.getAttribute(HsConstant.USER_KEY);
        ...
        {
            Log.log("ACTION_LAYER", DHSLogLevel.INFO, "HAZMAT VIEW ACTION",
                    "SEARCHSHIPMENTS", "Shipment ID: " + "IN LINE 212 ");
            System.out.println(filterSession.toString());

            System.out.println(filterSession.getValue() + " PRINTING VALUE");

            list = fwDAO.selectOrgList(HazmatPermitSQL.SELECT_ORG_BY_TIME_PERIOD,filterSession.getValue());
            Log.log("ACTION_LAYER", DHSLogLevel.INFO, "HAZMAT VIEW ACTION",
                    "SEARCHSHIPMENTS", "Shipment ID: " + "IN LINE 212 " + list);
            System.out.println(filterSession.toString() + " After retrieving DATA from SQL");
            System.out.println(list.size() + " LIST SIZE ");
            //orgForm.setState(filterSession.getValue());
        }
        else if(filterSession.getType().equals("byPermitNumber")){
            System.out.println(filterSession.getValue().equals("byPermitNumber"));
            list=fwDAO.selectOrgList(HazmatPermitSQL.SELECT_ORG_BY_PERMIT_NUMBER, filterSession.getValue());
        }
