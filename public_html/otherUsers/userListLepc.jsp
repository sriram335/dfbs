...
<h2>LEPC User's List</h2>
<c:if test="${sessionScope.DFBS_SECURITY.groupLevelAll != null   || sessionScope.DFBS_SECURITY.groupLevelLepc == 'USER'}"> 
<a   href="/dfbs/otherUsers/otherUser.do?method=addNewLepcUser">
              [add New User]</a>  

<div class="charNav">
<ul>
<c:forEach var="c" items="${USER_FIRST_LETTERS_OPTIONS_LEPC}" varStatus="i" >
<c:choose>
...
<c:url var="url" value="/otherUsers/otherUser.do?method=filterLepc&filter=byLetter&letter=${c.character}" />
...
<a   href="/dfbs/otherUsers/otherUser.do?method=emailLepcUsersInfo&userId=<c:out value="${user.userId}"/>">[Email New User Information]</a>
</c:if>
