...
<h2>Current Code Education courses </h2>
...
<c:url var="url" value="/codeEducation/course.do?method=filter&filter=byLetter&letter=${c.character}" />
...
<div id="leftContentFluid">
  ...
  <p class="listingInfo">
    ...
    clasee duration:<c:out value="${course.courseLength}" /><br />
    class Address:<c:out value="${course.address1}" />,<c:out value="${course.address2}" /><br />
                 <c:out value="${course.city}" />,&nbsp;<c:out value="${course.county}" />&nbsp;County,&nbsp;IN&nbsp;<c:out value="${course.zip}" /><br />
  ...
  <c:if test="${course.classVacancy > 0 && SECURITY_SUPER_USER !='YES'}">
   <a href="/dfbs/codeEducation/course.do?method=registerCourse&courseId=<c:out value="${course.courseId}" />">
              [Register for this class]  </a> </br>
  </c:if>
   <c:if test="${course.classVacancy <= 0}">
   Class Full, You can comeback later to see if any cancellations open the class for registration or contact bkendrick@dhs.in.gov for to get on the wait list. </br>
  </c:if>
  <c:if test="${SECURITY_SUPER_USER == 'YES'}">  
     <a href="/dfbs/codeEducation/course.do?method=courseRoster&courseId=<c:out value="${course.courseId}" />">
              [class roster]  </a> </br>
    <a href="https://oasdev.dhs.in.gov/reports/rwservlet?dfbsipnpdf&report=code_edu_class_roster&p_course_id=<c:out value="${course.courseId}" />">
