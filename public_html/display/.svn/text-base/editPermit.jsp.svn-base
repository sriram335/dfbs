...
<h2>Edit Fire Display Information</h2> 
<a   href="/dfbs/display/start.do?method=backToStart">
              [back to start]</a>
<a   href="/dfbs/display/start.do?method=renewBy">
              [back to search]</a>
<a   href="/dfbs/display/ownerListDisplay.do?method=renew&ownerId=<c:out value="${OWNER_SELECTED.ownerId}"/>&idNumber=<c:out value="${PERMIT_SELECTED.displayIdNumber}"/>">
              [back to owner]</a>
...
<table cellspacing="0" class="noBorder" summary="OWNER DATA">
 ...
 <%-- <c:if test="${sessionScope.DFBS_SECURITY.groupLevelAll != null   || sessionScope.DFBS_SECURITY.groupLevelFire != null }">  
  <tr>
      <th scope="row">display date from :</th>
      <td>
    ...
    <tr>
      <th scope="row">display date  to:</th>
      <td>
        ...
        <c:if test="${PERMIT_SELECTED.displayDatesMapCount > 0}">
      <tr>
