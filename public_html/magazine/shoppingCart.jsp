...
<%--
<p class="error">
Magzine online payment process is under maintenance, to renew or apply for new permits comeback later 
</p> --%>
...
<c:if test="${sessionScope.FILE_UPLOAD_STATUS =='N'}"> 
 <p class="error">
              Errors in the permits applied, check the cart for errors.
</p>

</c:if> <%--
<html:form action="/completeCheckout" method="post">
  <input type="hidden" name="method" id="METHOD_KEY" value="test"/>
      <p>
 ...
 <U><b>In this session</U></b></br>
  ...
  <c:forEach var="mapItem" items="${sessionScope.SHOPPING_CART.ownerMap}" varStatus="i">
      ...
      <tr class="row<c:out value='${(i.index % 2) + 1}' />">
       ...
       <c:if test="${sessionScope.SHOPPING_CART.receiptId == '0'}"> 
        <a   href="/dfbs/magazine/permit.do?method=addPermit&ownerKey=<c:out value="${owner.ownerKey}"/>">
         ...
         <span style="font-size:x-small">[remove owner]</span> 
          <a   href="/dfbs/magazine/permit.do?method=goToUploadOwner&ownerKey=<c:out value="${owner.ownerKey}"/>&permitKey=<c:out value="${permit.magPermitKey}"/>">
              <span style="font-size:x-small">[upload site plan(s) for owner]</span> </a></br>  
          </html:link> </br> 
           <c:if test="${owner.fileListCount == 0 }"> 
              <p class="error">
              Site Map needs to be uploaded.
              </p>
               </c:if>
          </c:if>
          ...
          Fees for this owner:<c:out value="${owner.amount}"/>0
           <c:forEach var="mapItem" items="${owner.permitsMap}" varStatus="j">
              ...
              <tr >
               ...
               <c:out value="${permit.magIdNumber}"/> magazine id/permit: <c:out value="${permit.magPermitKey}"/></br>
               magazine Type: <c:out value="${permit.magMagType}"/> </br>
               magazine Permit Fee: <c:out value="${permit.amount}"/>0</br>
              <c:if test="${sessionScope.SHOPPING_CART.receiptId == '0'}">
              <a   href="/dfbs/magazine/permit.do?method=editPermit&ownerKey=<c:out value="${owner.ownerKey}"/>&permitKey=<c:out value="${permit.magPermitKey}"/>">
              <span style="font-size:x-small">[edit permit]</span> </a>  
               <a   href="/dfbs/magazine/permit.do?method=removePermit&ownerKey=<c:out value="${owner.ownerKey}"/>&permitKey=<c:out value="${permit.magPermitKey}"/>">
              <span style="font-size:x-small">[remove permit]</span> </a>  
          <%--    <a   href="/dfbs/magazine/permit.do?method=goToUpload&ownerKey=<c:out value="${owner.ownerKey}"/>&permitKey=<c:out value="${permit.magPermitKey}"/>">
              <span style="font-size:x-small">[upload site plan]</span> </a></br>  --%>
              </c:if>
                </td>
                </tr>
              </c:forEach>
