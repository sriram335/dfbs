...
<a   href="/dfbs/aepermits/aepermits/inspection.do?method=updateInspection&inspectionId=<c:out value="${INSPECTION_SELECTED.inspId}"/>">
    ...
    <tr>
     ...
     <tr>
      <th scope="row"        >  violation remarks:</th>
       ...
       <tr>
          <th scope="row"  class="required"    > * violation to be correctd by Date:</th>
          <td>
           <html:text property="vioCorDate" size="10" maxlength="10"/>mm/dd/yyyy
            <c:if test="${INSPECTION_ERROR.vioCorDate == 'ERROR'}">
              <br/><span class="error">* please enter violation correctd by date  </span> 
            </c:if>
            </td>
          </th>
      </tr>
      <tr>
          <th scope="row"  class="required"    > * violation complied by Date:</th>
          <td>
           <html:text property="vioCompDate" size="8" maxlength="10"/>mm/dd/yyyy
