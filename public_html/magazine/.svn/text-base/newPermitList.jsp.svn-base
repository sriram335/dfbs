...
<h2>Update New Magazine Information</h2> 
...
<div id="leftContentFluid">
 ...
 <%-- <script type="text/javascript">
      setCheckFieldSize('MAG_COMMENTS');
    </script> --%>
 <html:form action="/permit" method="post">
      <input type="hidden" name="method" id="METHOD_KEY" value="updateNewPermit"/> 
   ...
   <tr>
    ...
    <tr>
     ...
     <tr>
      ...
      <tr>
        <th scope="row" >comments:(max 255 characters)1</th>
        <td>
            <html:textarea  property="magComments"  cols="70" rows="10" />
        </td>
    </tr>
     <tr>
...
<b>New Permits for Processing</b> </br> <a   href="/dfbs/magazine/start.do?method=lookupNewPermits">
                 [ refresh list ]</a> </br>
<c:forEach var="owner" items="${NEW_PERMITS_LIST.list}" varStatus="i" >
  ...
  <tbody>
    <c:forEach var="magazine" items="${owner.permits}" varStatus="i" >
      <tr class="row<c:out value='${(i.index % 2) + 1}' />">
        ...
        Application Date: <c:out value="${magazine.magSignedDateString}"/><br/>
   
      </td>
      <td>
           <c:if test="${ sessionScope.DFBS_SECURITY.groupLevelAll =='DBA' || sessionScope.DFBS_SECURITY.groupLevelFire != null}">
           <a   href="/dfbs/magazine/permit.do?method=editNewPermit&permitKey=<c:out value="${magazine.magIdNumber}"/>&ownerKey=<c:out value="${owner.ownerId}"/>">
                 [ edit new permit ]</a>
           </c:if>
