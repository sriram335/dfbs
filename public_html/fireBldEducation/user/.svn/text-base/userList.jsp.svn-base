...
<c:url var="url" value="/fireBldEducation/user.do?method=filter&filter=byLetter&letter=${c.character}" />
<a title="<c:out value='${c.count}' /> users" href="<c:out value='${url}' />"><c:out value='${c.character}' /></a>
  ...
  User Id:<a   href="/dfbs/fireBldEducation/user.do?method=updateUser&userPersonId=<c:out value="${user.userPersonId}"/>"><c:out value="${user.userId}" /> </a>
...
<c:if test="${SECURITY_SUPER_USER == 'YES' }">  
      <a   href="/dfbs/fireBldEducation/user.do?method=classList&userPersonId=<c:out value="${user.userPersonId}"/>">[Show Class List] </a>
  </c:if>
